function [] = save_individual_task_specific_cfc()
    ...
    task_char = ["Emotion","Gambling","Language","Motor","Relation","Social","Work_Memory"];
    scan_char = ["test","retest"];

    for task_i = 1:7
        for scan_index = 1:2
            fprintf("The task: %s; The scan: %s!\n", task_char{task_i}, scan_char{scan_index})
            HCP_task_data_filepath = ['C:\Holobrain\HCP_YA\HCP_task_data_', scan_char{scan_index}, '_labeled_', num2str(task_i),'.mat'];
            load(HCP_task_data_filepath)
            upd = textprogressbar(size(HCP_task_data,1), 'barlength', 20, ...
                            'updatestep', 10, ...
                            'startmsg', 'Waiting... ',...
                            'endmsg', ' Yay!', ...
                            'showbar', true, ...
            ...
            for sample_i = 1:size(HCP_task_data,1)
                Power = zeros(1440,size(HCP_task_data{sample_i,1},1));
                for fre_i = 1:4
                    Power(1+(fre_i-1)*360:fre_i*360,:) = subdict{1,fre_i}*HCP_task_data{sample_i,1}';
                end
                CFC = Power*Power';
                Individual_cfc_filepath = ['Individual_CFC_', HCP_task_data{sample_i,3}, '_', HCP_task_data{sample_i,2}, '.txt'];
                writematrix(CFC, Individual_cfc_filepath, 'Delimiter', 'tab')
                upd(sample_i)
            end
        end
    end
