function upd = textprogressbar(n, samplePid, varargin)
...
%
    ...
    addRequired(p, 'samplePid', @ischar)
    ...
    parse(p, n, samplePid, varargin{:});
    n = p.Results.n;
    samplePid = p.Results.samplePid;
    ...
    actualsamplePid = samplePid;
    actualNumFormat = sprintf(' %%%dd/%d %%s', actualNumDigitLen, n);
    actualNumStr = sprintf(actualNumFormat, 0, samplePid);
    ...
    delAll = repmat('\b', 1, arrayLen);
        ...
        function update(i, sapmleid)
            ...
            if showActualNum
                % Delete actual num block:
                actualNumStr = sprintf(actualNumFormat, i, sapmleid);
