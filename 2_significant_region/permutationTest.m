...
if strcmp(sidedness, 'both')
    tmp_length=length(find(abs(randomdifferences) > abs(observeddifference)));
    if norm(randomdifferences)<=0.000001
        p = 1;
    else
        p = (tmp_length+1) / (permutations+1);
    end
elseif strcmp(sidedness, 'smaller')
    tmp_length=length(find(randomdifferences < observeddifference));
    if norm(randomdifferences)<=0.000001
        p = 1;
    else
        p = (tmp_length+1) / (permutations+1);
